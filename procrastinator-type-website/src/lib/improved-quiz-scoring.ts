...
export interface ImprovedQuestion {
  ...
  // Add flag to control option randomization
  fixedOrder?: boolean;
  options: {
    text: string;
    scores: {
    ...
    // Add flag to identify special options
...
export interface ImprovedQuizResult {
  ...
  noneOfAboveCount: number; // Track neutral responses
...
export function calculateImprovedQuizResult(
  ...
  let noneOfAboveCount = 0; // Track "none of the above" responses

  answers.forEach(answer => {
    const question = questions.find(q => q.id === answer.questionId);
    if (question && answer.selectedOptionIndices && answer.selectedOptionIndices.length > 0) {
      
      answer.selectedOptionIndices.forEach(selectedIndex => {
        if (question.options[selectedIndex]) {
          const selectedOption = question.options[selectedIndex];
          const optionScores = selectedOption.scores;
          const weight = question.discriminantWeight || 1.0;
    
          // Track "none of the above" responses
          if ('isNoneOfAbove' in selectedOption && selectedOption.isNoneOfAbove) {
            noneOfAboveCount++;
          }
  ...
  const noneOfAbovePercentage = totalQuestions > 0 ? Math.round((noneOfAboveCount / totalQuestions) * 100) : 0;
  const confidenceLevel = getConfidenceLevel(scoreDifference, totalPossibleScore, consistencyScore, noneOfAbovePercentage);
...
function getConfidenceLevel(
  ...
  consistencyScore: number,
  noneOfAbovePercentage: number = 0
): 'high' | 'medium' | 'low' {
  const differenceRatio = scoreDifference / totalPossibleScore;

  // Reduce confidence if user selected too many "none of the above" (>30%)
  if (noneOfAbovePercentage > 30) {
    return 'low';
  }

  // High confidence: clear winner, high consistency, few neutral responses
  if (differenceRatio > 0.15 && consistencyScore > 85 && noneOfAbovePercentage < 20) {
    return 'high';
  }

  // Medium confidence: moderate difference or good consistency, reasonable neutral responses
  if ((differenceRatio > 0.08 || consistencyScore > 75) && noneOfAbovePercentage < 25) {
    return 'medium';
  }

  // Low confidence: close scores, poor consistency, or too many neutral responses
  return 'low';
}
