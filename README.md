...
### Technical SEO
- **`sitemap.xml` + `robots.txt`**: Auto-generated by `src/app/sitemap.ts` and `src/app/robots.ts` (all 16 routes; `/api/` and `/admin` disallowed). Submit `https://procrastitype.jnprojects.me/sitemap.xml` in Google Search Console and Bing Webmaster Tools.
- **Per-page metadata**: Unique titles, descriptions, canonical URLs, Open Graph and Twitter cards on every route.
- **Google Search Console verification**: `google-site-verification` meta tag in the site head (via Next metadata) plus `public/google87e4ddcc8e80c24b.html` as backup
...
```bash
NEXT_PUBLIC_SITE_URL=https://procrastitype.jnprojects.me   # canonical domain for sitemap/robots/schema
...
### SEO Next Steps
- [ ] Verify `https://procrastitype.jnprojects.me` in Google Search Console (verification file already deployed)
- [ ] Submit `sitemap.xml` in GSC + Bing Webmaster Tools and request indexing of `/types` pages
